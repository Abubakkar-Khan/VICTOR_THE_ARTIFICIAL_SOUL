...
from dexter.core.agent import DexterAgent, VictorAgent
from dexter.core.config import load_config
from dexter.core.events import AgentEvent, EventBus
from dexter.models.factory import create_model_provider
from dexter.permissions.manager import PermissionDecision

app = FastAPI(title="Dexter — The Artificial Soul API", version="1.0.0")
...
config = load_config()
agent = DexterAgent(config=config)

if getattr(sys, "frozen", False) and hasattr(sys, "_MEIPASS"):
    meipass = Path(sys._MEIPASS)
    web_dir = meipass / "dexter" / "web"
    if not web_dir.exists():
        web_dir = meipass / "victor" / "web"
else:
...
async def get_status():
    ...
    from dexter.core.agent import EMOTIONS
    return {
