...
import uvicorn
from dexter.desktop.mascot import launch_desktop_mascot
...
def wait_for_server(port: int, max_wait: float = 20.0) -> bool:
    ...
    while time.time() - start_time < max_wait:
        try:
            req = urllib.request.Request(status_url, headers={"User-Agent": "DexterLauncher"})
...
def main():
    ...
    port = int(os.environ.get("DEXTER_PORT", os.environ.get("VICTOR_PORT", "0"))) or find_available_port(8000)
    os.environ["DEXTER_PORT"] = str(port)
    ...
    icon_file = root_dir / "dexter" / "sprite" / "icon.ico"
    if not icon_file.exists():
        icon_file = root_dir / "victor" / "sprite" / "icon.ico"
    if not icon_file.exists() and getattr(sys, "frozen", False):
        exe_parent = Path(sys.executable).parent
        for candidate in [
            exe_parent / "dexter" / "sprite" / "icon.ico",
            exe_parent / "_internal" / "dexter" / "sprite" / "icon.ico",
            exe_parent / "victor" / "sprite" / "icon.ico",
            exe_parent / "_internal" / "victor" / "sprite" / "icon.ico",
        ]:
            if candidate.exists():
                icon_file = candidate
                break
